        ...
        try {
            if(req.query.location) {
                ...
                if(location.data.length) {
                    ...
                    const month = constants.Month[req.query.month] || d.getMonth() + 1;
                    d.setFullYear(year, month, 0);
                    const lastdate = d.getDate()
                    const { data } = await axios.get(`${constants.SERVER_URL.NASA_POWER}/api/temporal/daily/regional?latitude-min=${lat - 1}&latitude-max=${lat + 1}&longitude-min=${lon - 1}&longitude-max=${lon + 1}&parameters=T2M&community=SB&start=${year}${month}01&end=${year}${month}${lastdate}&format=JSON`);
                    let approxLat = 0;
                    let approxLon = 0;
                    if(lat%1 > 0.50) {
